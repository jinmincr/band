...
function highlightPage() {  //直接在当前页的html中加class属性不就好了
...
function prepareSection(){
	...
	for(var i=0;i<links.length;i++){
		...
		links[i].destination=sectionId;  /*sectionId是局部变量，作用域仅限此函数，不能直接传递到showSection*/
...
function highlightRows(){
	...
	for(var i=0;i<rows.length;i++){
		rows[i].oldClassName=rows[i].className;   /*自定义属性oldClassName*/
